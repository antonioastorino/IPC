...
public:
    Transmitter(const char* filePath, char secretChar) {
        if (Transmitter::s_initialized_) throw "Transmitter already initialized";
    ...
    void run() {
        ...
        while (!(clearMsg[0] == IPC::terminationChar && clearMsg[1] == '\n')) {
            ...
            for (size_t i = 0; i < clearMsg.size(); i++) {
                encodedMsg
                    += mp.getCodeForSymbol(clearMsg[i]); // Huffman's encoding for each character
...
int main() {
    ...
    IPC::Transmitter tx("../include/IPCConstants.hpp", 65);
    tx.run();
